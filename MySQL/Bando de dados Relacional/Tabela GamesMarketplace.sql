...
-- join
select tb_produtos.nome, tb_categorias.tipo -- pegando os dados
from tb_produtos inner join tb_categorias -- O tipo de junção
on tb_categorias.id = tb_produtos.categoria_id; -- Atributos para conectar

select tb_produtos.nome, tb_usuarios.nome as vendedor
from tb_produtos inner join tb_usuarios
on tb_usuarios.id = tb_produtos.usuarios_id;
